 ...
 */
export function serializeLayoutForSave(displayName: string, state: LayoutState): SavedLayoutData {
  const panes: SavedPaneData[] = state.panes.map((pane) => {
    ...
    if (pane.command) {
      const trimmed = pane.command.trim()
      if (trimmed) out.command = trimmed
    }
    return out
  })
